    ...
    def forward(
        ...
        if len(attention_mask.shape) == 4 :
            ...
            if not self.config.full_attn_mask:
                # not used in Dream
                logger.warning_once(
                    f"Use causal attention mask, self.config.full_attn_mask = {self.config.full_attn_mask}."
                )
                ...
                # used in Dream
                logger.warning_once(
                    f"Use full attention mask, self.config.full_attn_mask = {self.config.full_attn_mask}."
                )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if attention_mask_4d is None:
            ...
            if not use_cache:
                selected_attention_mask = attention_mask_4d
            else:
                if cache_position[0] == 0:
                    selected_attention_mask = attention_mask_4d
                else:
